    ...
    for transaction in transaction_list:
        if isinstance(transaction.entity, str) and transaction.transaction_type == TransactionType.Transfer and transaction.transaction_amount > Decimal("0"):
            if "Smit".lower() in transaction.entity.lower():
                jason_smit_reserve_account: ReserveAccount = ReserveAccount.get_reserve_account_by_profile_type_currency_and_name(nzd_account.user_profile.type, nzd_account.currency, "Jason Smit [Rent]", True)
                ...
                jason_smit_reserve_account = ReserveAccount.get_reserve_account_by_profile_type_currency_and_name(nzd_account.user_profile.type, nzd_account.currency, "Jason Smit [Rent]", True)
